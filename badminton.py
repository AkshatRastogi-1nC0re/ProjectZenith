...
def play_badminton():
    ...
    def MyEllipse(frame, x, y):
        ...
        return cv2.ellipse(frame, (x, y), (height, widthMotion), 90 - motion, 0, 360, (255, 0, 0), thickness,
                           line_type), cv2.ellipse(frame, (x, y), (height, widthMotion), 0 - motion, 0, 360, (255, 0, 0),
                                                   thickness, line_type), cv2.ellipse(frame, (x, y), (height, widthMotion),
                                                                                      45 - motion, 0, 360, (255, 0, 0),
                                                                                      thickness, line_type), cv2.ellipse(
            frame, (x, y), (height, widthMotion), -45 - motion, 0, 360, (255, 0, 0), thickness, line_type)
    ...
    while True:
        ...
        if (isStarted == True):
            if (isOver == False):
                cv2.putText(frame, 'SCORE: ' + str(score), (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2,
                            cv2.LINE_AA)
            elif (isOver == True):
            ...
            time = 100
            cv2.putText(frame, 'GAME WILL START IN --> ' + str(time - startingCounter), (200, 50), cv2.FONT_HERSHEY_SIMPLEX,
